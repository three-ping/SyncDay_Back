...
public class ChatService2 {
    ...
    public List<ChatRoomDTO> findUserChat(Long userId) {
        ...
        Map<Long, String> userNameMap = userRepository.findByUserIn(memberIds)
                .stream()
...
public ChatRoomDTO leaveChatRoom(String roomId, Long userId) {
    ...
    if (memberIds.isEmpty()) {
        ...
        ChatMessageDTO leaveMessage = ChatMessageDTO.builder()
                ...
                .build();
        chatMessageRepository.save(convertToChatMessage(leaveMessage));

        Map<Long, String> userNameMap = userRepository.findByUserIn(updatedChatRoom.getMemberIds());

        return ChatRoomDTO.from(updatedChatRoom, userNameMap);
    }
...
// 채팅방 이름 수정(단체톡방 일 시)
    public ChatRoomDTO updateRoomName(String roomId, String newRoom){
        ...
        Map<Long, String> userNameMap = userRepository.findByUserIn(chatRoom.getMemberIds())
                .stream()
                .collect(Collectors.toMap(UserEntity::getUserId, UserEntity::getUserName));
